...
## 4:50:25 PM
**File: `g:\wamp64\www\sitenettech\api-client\src\pages\backend\Folder\FolderList.jsx`**
This file, representing the Folder List view, shows no visible functional code changes across multiple entries from `1/25/2026, 11:14:23 AM` to `1/25/2026, 2:49:42 PM`. The consistent truncation of the code snippet suggests that either no modifications were made to the visible parts of the file during this period, or changes were minor and not captured in the logs. The file uses `formik` for form handling, `primereact/toast` and `flowbite-react/Tooltip` for UI, and integrates with Supabase for data management. It defines various state variables for data fetching, filtering, pagination, and UI modals (e.g., `loading`, `tableData`, `openFilter`, `deleteModalOpen`). It also includes functionality for creating new folders and applying filters based on predefined queries.

**File: `g:\wamp64\www\sitenettech\api-client\src\components\Backend\Interface\BookingDetails\parts\BookingDetailsButtons.jsx`**
A single entry at `1/25/2026, 12:09:27 PM` reveals a component responsible for adding booking details to a folder. The core functionality is encapsulated in an asynchronous `AddToFolder` function. This function:
*   Extracts booking information (contact details, flights, hotels, transfers, etc.).
*   Checks for an existing folder with the same booking reference. If none exists, it creates a new folder with a generated UID, initial status "open," and lead passenger details.
*   Populates `passengers` data, including generating UIDs for unnamed passengers, and inserts them into the Supabase database.
*   Processes `airsegments`, maps their statuses (e.g., CONFIRMED to HK), and inserts them.
*   Includes a detailed pricing logic (`buildPricePoolByType`, `buildPassengerPricing`) to calculate buy, sell, commission, and VAT for passengers based on different passenger types (Adult, Child, Infant).
*   Handles navigation to the folder edit page upon successful creation and provides error feedback using `showError`.

**File: `g:\wamp64\www\sitenettech\api-client\src\pages\backend\Folder\FolderReview\FolderReview.jsx`**
This file, handling the Folder Review interface, underwent a series of focused updates between `1/25/2026, 12:26:16 PM` and `1/25/2026, 4:38:06 PM`:
*   **Initial Setup (12:26:16 PM):** The component initializes various states related to UI collapse, loading, data display, active tabs, and travel options. It uses `useLocation` to retrieve parsed data (`propsData`) and stores it in `sessionStorage` for persistence. Utility functions for data processing (`getFolderOptions`) and user/company information fetching are present.
*   **PNR Management (3:41:06 PM - 3:52:59 PM):**
    *   State variables `pnrInput` and `pnrError` were introduced to manage user input for PNR (Passenger Name Record) data. `pnrInput` is initially set from existing `air_segment_pnr` data.
    *   A `useEffect` hook was added to ensure `pnrInput` dynamically updates when `folderData` changes, reflecting accurate PNR information.
    *   `ModalCustom` was imported, and `showPNRModal` state was added to control the visibility of a PNR entry modal.
    *   `modalShownRef` was implemented to ensure the PNR modal appears only once if `pnrInput` is empty, preventing repeated pop-ups.
*   **UI/Utility Imports (3:56:17 PM - 4:05:07 PM):**
    *   `ButtonFilter` component was imported.
    *   `Toast` from `primereact/toast` and `showError` from `toastify` utility were imported, indicating an enhancement in user feedback and error display within this component.
*   **Travel Option Handling (4:24:51 PM - 4:38:06 PM):** The logic for setting `travelOption` was changed. Instead of processing `propsData` through the `getFolderOptions` utility, it now directly uses `propsData?.folder?.travel_options` as its source. This suggests a more direct data structure for travel options is now expected or provided.

**Patterns and Recurring Elements:**
*   **Supabase Integration:** Both `FolderList.jsx` and `BookingDetailsButtons.jsx` (and implicitly `FolderReview.jsx` through its user/company data fetching) heavily rely on Supabase for data storage and real-time updates.
*   **React Hooks:** Extensive use of `useState`, `useEffect`, and `useRef` for managing component lifecycle, local state, and preventing re-renders.
*   **Form Management:** `useFormik` is employed in `FolderList.jsx` for form validation and submission, indicating a structured approach to user input.
*   **UI Component Reusability:** Custom UI components (e.g., `Button`, `ModalCustom`, `Pageheader`, `DataTableList`) are consistently imported, suggesting a component-based design system.
*   **Modular Code Structure:** The breakdown into `Parts` folders (e.g., `FolderCreateBody`, `FolderFilterBody` in `FolderList.jsx`, and `AirSegments`, `AirTickets`, `Passengers` in `FolderReview.jsx`) emphasizes modularity.
*   **Error Reporting:** A consistent pattern of using `primereact/toast` and custom `toastify` utilities (`showError`, `showSuccess`) for user feedback on operations.
*   **Data Persistence:** Usage of `sessionStorage` in `FolderReview.jsx` to persist `folderReviewData` across sessions or component reloads.
*   **Icon Library:** A wide array of `react-icons` are imported across both `FolderList.jsx` and `FolderReview.jsx`, suggesting a rich iconographic user interface.